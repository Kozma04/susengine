...
// Returns true if two colliders are intersecting. Has optional Minimum
// Translation Vector output param; If supplied the EPA will be used to find the
// vector to separate coll1 from coll2
...
static void update_simplex3(Point *a, Point *b, Point *c, Point *d,
                            int *simp_dim, Vector3 *search_dir);
static uint8_t update_simplex4(Point *a, Point *b, Point *c, Point *d,
                               int *simp_dim, Vector3 *search_dir);
...
static uint8_t EPA(Point *a, Point *b, Point *c, Point *d,
                   GJKColliderMesh *coll1, GJKColliderMesh *coll2, Vector3 *nor,
                   Vector3 *locA, Vector3 *locB);
...
static Vector3 supportVec(GJKColliderMesh *mesh, Vector3 dir) {
    ...
    for (int i = 0; i < mesh->nVertices * 3; i += 3) {
        vert = (Vector3){mesh->vertices[i], mesh->vertices[i + 1],
                         mesh->vertices[i + 2]};
...
static void calculateSearchPoint(Point *point, Vector3 dir,
                                 GJKColliderMesh *coll1,
                                 GJKColliderMesh *coll2) {
...
static uint8_t gjk(GJKColliderMesh *coll1, GJKColliderMesh *coll2, Vector3 *mtv,
                   Vector3 *contactA, Vector3 *contactB) {
    Point a, b, c,
        d; // Simplex: just a set of points (a is always most recently added)
    Vector3 search_dir = Vector3Subtract(
        coll1->pos, coll2->pos); // initial search direction between colliders
    ...
    // search_dir = cross(cross(c-b,-b),c-b); //search perpendicular to line
    // segment towards origin
    search_dir =
        Vector3CrossProduct(Vector3Subtract(c.p, b.p), Vector3Negate(b.p));
    search_dir = Vector3CrossProduct(search_dir, Vector3Subtract(c.p, b.p));
    if (search_dir.x == 0 && search_dir.y == 0 &&
        search_dir.z == 0) { // origin is on this line segment
        // Apparently any normal search vector will do?
        // search_dir = cross(c-b, vec3(1,0,0)); //normal with x-axis
        search_dir =
            Vector3CrossProduct(Vector3Subtract(c.p, b.p), (Vector3){1, 0, 0});
        // if(search_dir==vec3(0,0,0)) search_dir = cross(c-b, vec3(0,0,-1));
        // //normal with z-axis
        if (search_dir.x == 0 && search_dir.y == 0 && search_dir.z == 0) {
            search_dir = Vector3CrossProduct(Vector3Subtract(c.p, b.p),
                                             (Vector3){0, 0, -1});
        }
    }
    int simp_dim = 2; // simplex dimension

    for (int iterations = 0; iterations < GJK_MAX_NUM_ITERATIONS;
         iterations++) {
        ...
        if (Vector3DotProduct(a.p, search_dir) < 0) {
            return 0;
        ...
        if (simp_dim == 3) {
            ...
            if (mtv && contactA && contactB) {
                if (EPA(&a, &b, &c, &d, coll1, coll2, &mtvInt, &contactAInt,
                        &contactBInt)) {
                    *mtv = mtvInt;
...
static void update_simplex3(Point *a, Point *b, Point *c, Point *d,
    ...
    */
    ...
    Vector3 n = Vector3CrossProduct(Vector3Subtract(b->p, a->p),
    ...
    *simp_dim = 2;
    ...
    float dp = Vector3DotProduct(
        Vector3CrossProduct(Vector3Subtract(b->p, a->p), n), AO);
    if (dp > 0) { // Closest to edge AB
        ...
        *search_dir =
            Vector3CrossProduct(*search_dir, Vector3Subtract(b->p, a->p));
        return;
    }
    // float dp = dot(cross(n, c-a), AO);
    dp = Vector3DotProduct(Vector3CrossProduct(n, Vector3Subtract(c->p, a->p)),
                           AO);
    if (dp > 0) { // Closest to edge AC
        ...
        *search_dir =
            Vector3CrossProduct(*search_dir, Vector3Subtract(c->p, a->p));
        return;
    }
...
static uint8_t update_simplex4(Point *a, Point *b, Point *c, Point *d,
                               int *simp_dim, Vector3 *search_dir) {
    // a is peak/tip of pyramid, BCD is the base (counterclockwise winding
    // order) We know a priori that origin is above BCD and below a

    // Get normals of three new faces
    Vector3 ABC = Vector3CrossProduct(Vector3Subtract(b->p, a->p),
                                      Vector3Subtract(c->p, a->p));
    Vector3 ACD = Vector3CrossProduct(Vector3Subtract(c->p, a->p),
                                      Vector3Subtract(d->p, a->p));
    Vector3 ADB = Vector3CrossProduct(Vector3Subtract(d->p, a->p),
    ...
    *simp_dim = 3;                    // hoisting this just cause
    ...
    /*
    // Note: Kind of primitive approach used here; If origin is in front of a
    face, just use it as the new simplex.
    // We just go through the faces sequentially and exit at the first one which
    satisfies dot product. Not sure this
    // is optimal or if edges should be considered as possible simplices?
    Thinking this through in my head I feel like
    // this method is good enough. Makes no difference for AABBS, should test
    with more complex colliders.
    */
    ...
    // Right now I don't think it'll make a difference to limit our new
    // simplices to just one of the faces, maybe test it later.
}
...
static void GetFaceNormals(Vector3 *polytope, int *faces, Vector4 *n,
                           int *minFace, int nFaces) {
    Vector3 a, b, c;
    Vector3 nor;
    int mf = 0;
...
static void AddIfUniqueEdge(Edge *edges, int *faces, int *nEdges, int a,
                            int b) {
...
static uint8_t EPA(Point *a, Point *b, Point *c, Point *d,
                   GJKColliderMesh *coll1, GJKColliderMesh *coll2, Vector3 *nor,
                   Vector3 *locA, Vector3 *locB) {
    Point faces[EPA_MAX_NUM_FACES]
               [4]; // Array of faces, each with 3 verts and a normal
    ...
    for (int iterations = 0; iterations < EPA_MAX_NUM_ITERATIONS;
         iterations++) {
        ...
        if (Vector3DotProduct(p.p, search_dir) - min_dist < EPA_TOLERANCE) {
            Plane closestPlane =
                PlaneFromTri(faces[closest_face][0].p, faces[closest_face][1].p,
                             faces[closest_face][2].p);
            Vector3 projectionPoint =
                ProjectPointOntoPlane(closestPlane, (Vector3){0, 0, 0});
            Vector3 bar = Vector3Barycenter(
                projectionPoint, faces[closest_face][0].p,
                faces[closest_face][1].p, faces[closest_face][2].p);

            Vector3 localA = Vector3Scale(faces[closest_face][0].a, bar.x);
            localA = Vector3Add(localA,
                                Vector3Scale(faces[closest_face][1].a, bar.y));
            localA = Vector3Add(localA,
                                Vector3Scale(faces[closest_face][2].a, bar.z));

            Vector3 localB = Vector3Scale(faces[closest_face][0].b, bar.x);
            localB = Vector3Add(localB,
                                Vector3Scale(faces[closest_face][1].b, bar.y));
            localB = Vector3Add(localB,
                                Vector3Scale(faces[closest_face][2].b, bar.z));
        ...
        Point loose_edges[EPA_MAX_NUM_LOOSE_EDGES]
                         [2]; // keep track of edges we need to fix after
                              // removing faces
        ...
        for (int i = 0; i < num_faces; i++) {
            ...
            float dp = Vector3DotProduct(faces[i][3].p,
                                         Vector3Subtract(p.p, faces[i][0].p));
            if (dp > 0) // triangle i faces p, remove it
            {
                // Add removed triangle's edges to loose edge list.
                // If it's already there, remove it (both triangles it belonged
                // to are gone)
                for (int j = 0; j < 3; j++) // Three edges per face
                {
                    Point current_edge[2] = {faces[i][j],
                                             faces[i][(j + 1) % 3]};
                    int found_edge = 0;
                         ...
                         k++) // Check if current edge is already in list
                    {
                        ...
                        if (Vector3Equals(loose_edges[k][1].p,
                                          current_edge[0].p) &&
                            Vector3Equals(loose_edges[k][0].p,
                                          current_edge[1].p)) {
                            // Edge is already in the list, remove it
                            // THIS ASSUMES EDGE CAN ONLY BE SHARED BY 2
                            // TRIANGLES (which should be true) THIS ALSO
                            // ASSUMES SHARED EDGE WILL BE REVERSED IN THE
                            // TRIANGLES (which should be true provided every
                            // triangle is wound CCW)
                            loose_edges[k][0] =
                                loose_edges[num_loose_edges - 1]
                                           [0]; // Overwrite current edge
                            loose_edges[k][1] =
                                loose_edges[num_loose_edges - 1]
                                           [1]; // with last edge in list
                            num_loose_edges--;
                            found_edge = 1;
                            k = num_loose_edges; // exit loop because edge can
                                                 // only be shared once
                        }
                    } // endfor loose_edges
        ...
        for (int i = 0; i < num_loose_edges; i++) {
            ...
            faces[num_faces][2] = p;
            // faces[num_faces][3] =
            // normalise(cross(loose_edges[i][0]-loose_edges[i][1],
            // loose_edges[i][0]-p));
            ...
            float bias = 0.000001; // in case dot result is only slightly < 0
                                   // (because origin is on face)
            if (Vector3DotProduct(faces[num_faces][0].p,
                                  faces[num_faces][3].p) +
                    bias <
                0) {
    ...
    Vector3 projectionPoint =
        ProjectPointOntoPlane(closestPlane, (Vector3){0, 0, 0});
    Vector3 bar =
        Vector3Barycenter(projectionPoint, faces[closest_face][0].p,
                          faces[closest_face][1].p, faces[closest_face][2].p);

    Vector3 localA = Vector3Scale(faces[closest_face][0].a, bar.x);
    localA = Vector3Add(localA, Vector3Scale(faces[closest_face][1].a, bar.y));
    localA = Vector3Add(localA, Vector3Scale(faces[closest_face][2].a, bar.z));

    Vector3 localB = Vector3Scale(faces[closest_face][0].b, bar.x);
    localB = Vector3Add(localB, Vector3Scale(faces[closest_face][1].b, bar.y));
    localB = Vector3Add(localB, Vector3Scale(faces[closest_face][2].b, bar.z));
