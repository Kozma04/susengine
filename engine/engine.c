...
void engine_entityPostCreate(Engine *const engine, const ECSEntityID id) {
    ...
    ecs_execCallbackAllComp(&engine->ecs, id, ENGINE_CB_CREATE,
                            &cbData);
}
...
void engine_entityDestroy(Engine *const engine, const ECSEntityID id) {
    EngineCallbackData cbData;
    cbData.engine = engine;
    ecs_execCallbackAllComp(&engine->ecs, id, ENGINE_CB_DESTROY,
                            &cbData);
...
static void engine_dispatchMessage(Engine *const engine, EngineMsg *const msg) {
    ...
    if(msg->dstId == ECS_INVALID_ID) {
        for(ent = 0; ent < engine->ecs.nActiveEnt; ent++) {
            ...
            if((compData->info.typeMask & msg->dstMask) == msg->dstMask) {
                ecs_execCallbackAllComp(
                    &engine->ecs, ent, ENGINE_CB_MSGRECV, 
                    &cbData
                );
            }
        }
        ...
        ecs_execCallbackAllComp(
            &engine->ecs, msg->dstId, ENGINE_CB_MSGRECV,
            &cbData
        );
    }
}
...
static void engine_execUpdateCallbacks(Engine *const engine,
    ...
    ecs_execCallbackAllEnt(
        &engine->ecs, ENGINE_CB_UPDATE, &cbData
    );
}
...
static void engine_updateTransforms(Engine *const engine) {
    ...
    for(uint32_t i = 0; i < engine->ecs.nActiveEnt; i++) {
        ...
        stat = ecs_getCompData(
            &engine->ecs, entId, ENGINE_COMP_TRANSFORM, (void**)&cData
        );
        if(cData == NULL) {
...
EngineStatus engine_createTransform(
    ...
    if(ecs_registerComp(&engine->ecs, ent, type, compRaw) == ECS_RES_OK) {
        ecs_setCallback(
            &engine->ecs, ent, type, ENGINE_CB_CREATE,
...
EngineStatus engine_createMeshRenderer(
    ...
    if(transformAnchor)
        ...
        res = ecs_getCompData(
            &engine->ecs, ent, ENGINE_COMP_TRANSFORM,
    ...
    if(ecs_registerComp(&engine->ecs, ent, type, compRaw) == ECS_RES_OK) {
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_CREATE, engine_cbMeshRendererOnCreate
        ); 
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_DESTROY, engine_cbMeshRendererOnDestroy
        ); 
        return ENGINE_STATUS_OK;
    }
    return ENGINE_STATUS_REGISTER_FAILED;
...
EngineStatus engine_createAmbientLight(
    ...
    if(ecs_registerComp(&engine->ecs, ent, type, compRaw) == ECS_RES_OK) {
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_CREATE, engine_cbLightSourceOnCreate
        ); 
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_DESTROY, engine_cbLightSourceOnDestroy
        );
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_UPDATE, engine_cbLightSourceOnUpdate
        );
        return ENGINE_STATUS_OK;
    }
    return ENGINE_STATUS_REGISTER_FAILED;
...
EngineStatus engine_createRigidBody(
    ...
    *comp = physics_initRigidBody(mass);
    ...
    if(ecs_registerComp(&engine->ecs, ent, type, compRaw) == ECS_RES_OK) {
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_CREATE, engine_cbRigidBodyOnCreate
        );
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_UPDATE, engine_cbRigidBodyOnUpdate
        );
        ecs_setCallback(
            &engine->ecs, ent, type,
            ENGINE_CB_DESTROY, engine_cbRigidBodyOnDestroy
        );
        return ENGINE_STATUS_OK;
    }
    return ENGINE_STATUS_REGISTER_FAILED;
...
EngineCompInfo *engine_getInfo(Engine *const engine, const ECSEntityID ent) {
    ...
    const ECSStatus res = ecs_getCompData(
        &engine->ecs, ent, ENGINE_COMP_INFO, (void**)&data
    );
    if(res != ECS_RES_OK)
        return NULL;
    return &data->info;
...
EngineCompCamera *engine_getCamera(
    ...
    EngineECSCompData *data;
    const ECSStatus res = ecs_getCompData(
        &engine->ecs, ent, ENGINE_COMP_CAMERA, (void**)&data
    );
    if(res != ECS_RES_OK)
        return NULL;
    return &data->cam;
...
Collider *engine_getCollider(Engine *engine, ECSEntityID ent) {
    EngineECSCompData *data;
    const ECSStatus res = ecs_getCompData(
        &engine->ecs, ent, ENGINE_COMP_COLLIDER, (void**)&data
    );
    if(res != ECS_RES_OK)
        return NULL;
    return &data->coll;
