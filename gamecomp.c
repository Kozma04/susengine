...
static void game_cbPlayerControllerOnUpdate(uint32_t cbType, ECSEntityID entId,
    ...
    pc->camForward =
        Vector3RotateByAxisAngle(pc->camForward, (Vector3){0, -1, 0}, md.x);
    ...
    if (Vector3DotProduct(
            ...
            Vector3CrossProduct(prevForward, (Vector3){0, 1, 0})) < 0) {
        Vector2 dirXZ =
            Vector2Normalize((Vector2){prevForward.x, prevForward.z});
    ...
    if (pc->mode == GAME_PLAYERMODE_NOCLIP) {
        ...
        deltaPos =
            Vector3Scale(deltaPos, pc->moveSpeed * cbData->update.deltaTime);

        actualDeltaPos = Vector3Add(
            actualDeltaPos,
            Vector3Scale(Vector3Subtract(deltaPos, actualDeltaPos), 0.1));

        rb->pos = Vector3Add(trans->pos, actualDeltaPos);
        trans->localUpdate = 1;
    } else {
        uint8_t fast =
            IsKeyDown(KEY_LEFT_SHIFT) && Vector3Length(deltaPos) > .5f;
        ...
        cam->cam.fovy +=
            (camFOV - cam->cam.fovy) * 10 * cbData->update.deltaTime;
    }
...
static void
engine_cbCollisionDbgViewOnUpdate(uint32_t cbType, ECSEntityID entId,
                                  ECSComponentID compId, uint32_t compType,
                                  struct ECSComponent *comp, void *cbUserData) {
    ...
    if (coll->nContacts > 0) {
        ...
        for (int i = 0; i < coll->nContacts; i++) {
            logMsg(LOG_LVL_INFO, "%u collides with %u", entId,
                   coll->contacts[i].targetId);
        }
    } else {
...
static void weatherCbPreRender(uint32_t cbType, ECSEntityID entId,
                               ECSComponentID compId, uint32_t compType,
                               struct ECSComponent *comp, void *cbUserData) {
    ...
    SetShaderValue(cbData->render.shader,
                   GetShaderLocation(cbData->render.shader, "environmentMap"),
                   &texSlot, SHADER_UNIFORM_INT);
}

static void weatherCbPostRender(uint32_t cbType, ECSEntityID entId,
                                ECSComponentID compId, uint32_t compType,
                                struct ECSComponent *comp, void *cbUserData) {
...
static void waterCbPreRender(uint32_t cbType, ECSEntityID entId,
                             ECSComponentID compId, uint32_t compType,
                             struct ECSComponent *comp, void *cbUserData) {
    EngineCallbackData *cbData = (EngineCallbackData *)cbUserData;
    EngineCompMeshRenderer *mr = &((EngineECSCompData *)comp->data)->meshR;
    Model *mdl = engine_render_getModel(cbData->engine, mr->modelId);
    EngineCompCamera *cam = (EngineCompCamera *)cbData->engine->ecs
                                .comp[cbData->engine->render.camera]
                                .data;
    ...
    SetShaderValue(cbData->render.shader,
                   GetShaderLocation(cbData->render.shader, "texDiffuse"),
                   &texSlot, SHADER_UNIFORM_INT);
    SetShaderValue(cbData->render.shader,
                   GetShaderLocation(cbData->render.shader, "time"), &time,
                   SHADER_UNIFORM_FLOAT);
    SetShaderValue(cbData->render.shader,
                   GetShaderLocation(cbData->render.shader, "cameraPosition"),
                   &cam->cam.position, SHADER_UNIFORM_VEC3);
}

static void engine_createCollisionDbgView(Engine *const engine,
...
static void boxPropCustomCallback(uint32_t cbType, ECSEntityID entId,
                                  ECSComponentID compId, uint32_t compType,
                                  struct ECSComponent *comp, void *cbUserData) {
    EngineCallbackData *cbData = cbUserData;
...
static void gameCreatePlayerController(Engine *engine, ECSEntityID ent) {
    ...
    ecs_registerComp(&engine->ecs, ent, GAME_COMP_CONTROLLER, controller);
    ecs_setCallback(&engine->ecs, ent, GAME_COMP_CONTROLLER, ENGINE_CB_UPDATE,
                    game_cbPlayerControllerOnUpdate);
}
...
Player createPlayer(Engine *engine) {
    ...
    engine_createConvexHullColliderModel(engine, player.id,
                                         GAME_MODEL_CYLINDER);
    ...
    ecs_getCompData(&engine->ecs, player.id, GAME_COMP_CONTROLLER,
                    (void **)&ctrl);
    ...
    player.coll->localTransform =
        MatrixMultiply(MatrixScale(1, 1.8, 1), MatrixTranslate(0, -1.8, 0));
...
Prop createProp(Engine *engine, EngineRenderModelID modelId) {
    ...
    ecs_setCallback(&engine->ecs, prop.id, ENGINE_COMP_RIGIDBODY,
